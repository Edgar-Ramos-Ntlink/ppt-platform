...
public class FacturaDao {
  ...
  private static final String INVOICE_BY_FOLIO =
      "SELECT "
              + "	f.FOLIO,f.LINEA_EMISOR,cts.CORREO_PROMOTOR,cts.PORCENTAJE_CLIENTE,cts.PORCENTAJE_CONTACTO,cts.PORCENTAJE_DESPACHO,cts.PORCENTAJE_PROMOTOR,f.UUID,rep.FECHA,f.RAZON_SOCIAL_EMISOR,f.RFC_EMISOR,f.RAZON_SOCIAL_REMITENTE,f.RFC_REMITENTE,f.TIPO_DOCUMENTO,f.PACK_FACTURACION,rep.TIPO_COMPROBANTE,"
              + "					rep.IMP_TRASLADADOS,rep.IMP_RETENIDOS,rep.SUB_TOTAL,rep.TOTAL,f.SALDO_PENDIENTE,rep.METODO_PAGO,rep.FORMA_PAGO,rep.MONEDA,f.STATUS_FACTURA,f.FECHA_CANCELADO,rep.CANTIDAD,rep.CLAVE_UNIDAD,rep.UNIDAD,"
              + "					rep.CLAVE_PROD_SERV,rep.DESCRIPCION,rep.VALOR_UNITARIO,rep.IMPORTE FROM FACTURAS f INNER JOIN REPORTES rep ON f.FOLIO = rep.FOLIO "
  ...
  private static final String INVOICES_BY_FOLIOS =
      "SELECT "
              + "	f.FOLIO,f.LINEA_EMISOR,cts.CORREO_PROMOTOR,cts.PORCENTAJE_CLIENTE,cts.PORCENTAJE_CONTACTO,cts.PORCENTAJE_DESPACHO,cts.PORCENTAJE_PROMOTOR,f.UUID,rep.FECHA,f.RAZON_SOCIAL_EMISOR,f.RFC_EMISOR,f.RAZON_SOCIAL_REMITENTE,f.RFC_REMITENTE,f.TIPO_DOCUMENTO,f.PACK_FACTURACION,rep.TIPO_COMPROBANTE,"
              + "					rep.IMP_TRASLADADOS,rep.IMP_RETENIDOS,rep.SUB_TOTAL,rep.TOTAL,f.SALDO_PENDIENTE,rep.METODO_PAGO,rep.FORMA_PAGO,rep.MONEDA,f.STATUS_FACTURA,f.FECHA_CANCELADO,rep.CANTIDAD,rep.CLAVE_UNIDAD,rep.UNIDAD,"
              + "					rep.CLAVE_PROD_SERV,rep.DESCRIPCION,rep.VALOR_UNITARIO,rep.IMPORTE "
              + "FROM FACTURAS f "
              + "INNER JOIN REPORTES rep ON f.FOLIO = rep.FOLIO "
              + "LEFT JOIN CLIENTES  cts ON cts.RFC = f.RFC_REMITENTE "
              + "WHERE rep.FOLIO IN (%s)";

  private static final String COMPLEMENT_BY_FOLIO =
      "SELECT p.ID_DOCUMENTO AS UUID,rep.FECHA,f.RAZON_SOCIAL_EMISOR,f.RFC_EMISOR,f.RAZON_SOCIAL_REMITENTE,f.RFC_REMITENTE,f.TIPO_DOCUMENTO,f.PACK_FACTURACION,"
              + "	rep.TIPO_COMPROBANTE,rep.IMP_TRASLADADOS,rep.IMP_RETENIDOS,rep.SUB_TOTAL,rep.TOTAL,p.MONEDA,p.FORMA_PAGO,rep.METODO_PAGO,f.STATUS_FACTURA,f.FECHA_CANCELADO,f.FOLIO AS FOLIO_PAGO,f.UUID AS UUID_PAGO,"
              + " p.IMPORTE_PAGADO,p.IMPORTE_SALDO_ANTERIOR,p.IMPORTE_SALDO_INSOLUTO,p.NUM_PARCIALIDAD,p.FECHA_PAGO FROM FACTURAS f INNER JOIN REPORTES rep ON f.FOLIO = rep.FOLIO INNER JOIN CFDI_PAGOS p ON rep.FOLIO = p.FOLIO "
              + "WHERE rep.FOLIO= ?";
