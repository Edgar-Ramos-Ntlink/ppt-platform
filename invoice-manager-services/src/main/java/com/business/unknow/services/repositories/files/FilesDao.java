...
public class FilesDao {
  ...
  private static final String FIND_FACTURA_FILE_BY_FOLIO_AND_TYPE =
      "SELECT * FROM FACTURA_FILES WHERE FOLIO = ? AND TIPO_ARCHIVO = ?";

  private static final String DELETE_FACTURA_FILE_BY_FOLIO_AND_TYPE =
      "DELETE FROM FACTURA_FILES WHERE FOLIO = ? AND TIPO_ARCHIVO = ?";

  private static final String DELETE_FACTURA_FILE_BY_ID =
      "DELETE FROM FACTURA_FILES WHERE FILE_ID=?";

  private static final String INSERT_FACTURA_FILE =
      "INSERT INTO FACTURA_FILES (FOLIO, TIPO_ARCHIVO, DATA, FECHA_CREACION) VALUES(?,?,?,?)";
  ...
  public Optional<FacturaFileDto> findFacturaFileByResourceTypeAndReference(
      String folio, String fileType) {
    return invoiceManagerTemplate.query(
        new PreparedStatementCreator() {
          @Override
          public PreparedStatement createPreparedStatement(Connection con) throws SQLException {
