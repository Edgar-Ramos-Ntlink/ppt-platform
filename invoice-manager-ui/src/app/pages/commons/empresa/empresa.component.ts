...
export class EmpresaComponent implements OnInit {
  ...
  public async loadCompanyInfo(rfc: string): Promise<void> {
    ...
    try {
      ...
      this.observaciones =
        this.companyInfo.detalles.filter((d) => d.tipo === "OBSERVACION") || [];
      this.pendientes =
        this.companyInfo.detalles.filter((d) => d.tipo === "PENDIENTE") || [];
      this.accionistas =
        this.companyInfo.detalles.filter((d) => d.tipo === "ACCIONISTA") || [];
      this.apoderados =
        this.companyInfo.detalles.filter((d) => d.tipo === "APODERADO") || [];
  ...
  private async loadDocuments(rfc: string) {
    ...
    this.legalDocuments = documents.filter(
      (d) => this.CONTABLE_FILES.find((c) => c == d.tipoArchivo) == undefined
    );
    ...
    if (this.legalDocuments.find((d) => d.tipoArchivo === "LOGO")) {
      ...
      this.resourcesService.getResourceFile(rfc, "EMPRESAS", "LOGO").subscribe(
        (logo) => (this.logo = logo),
        (error) => {
          let msg =
            error.error.message || `${error.statusText} : ${error.message}`;
  ...
  public tooglePass() {
    this.formInfo.showPass = !this.formInfo.showPass;
  }
  ...
  public onBlockSelected(blocked: boolean) {
    this.companyInfo.blocked = blocked;
  }
    ...
    if (event.target.files && event.target.files.length > 0) {
      ...
      if (file.size > 200000) {
        ...
        reader.onload = () => {
          ...
          this.resourcesService.insertResourceFile(this.logo).subscribe(
            () => {
              this.showToast(
                "info",
                "Exito!",
  ...
  public fileDataUploadListener(event: any): void {
    ...
    if (event.target.files && event.target.files.length > 0) {
      ...
      reader.onerror = (error) => {
        this.showToast("danger", "Error", "Error cargando el archivo", true);
      };
    }
  }
