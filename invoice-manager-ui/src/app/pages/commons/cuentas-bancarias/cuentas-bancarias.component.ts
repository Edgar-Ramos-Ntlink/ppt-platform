...
export class CuentasBancariasComponent implements OnInit {
    ...
    this.route.queryParams.subscribe((params) => {
      if (!this.utilsService.compareParams(params, this.filterParams)) {
        ...
        this.catalogsService
          .getBancos()
          .then((banks) => (this.banksCat = banks));

        if (params.empresa) {
          this.companiesService
            .getCompanyByRFC(params.empresa)
            .subscribe((company: Empresa) => {
              this.filterParams.razonSocial = company.razonSocial;
            });
        }
  ...
  public downloadHandler() {
    ...
    params.size = 10000;
    this.accountsService
      .getCuentasReport(params.page, params.size, params)
      .subscribe((result) => {
        this.downloadService.downloadFile(
          result.data,
          "ReporteCuentasBancarias.xlsx",
  ...
  getSelector($event) {
    let inputValue = (<HTMLInputElement>document.getElementById("empresasId"))
      .value;
    this.listEmpresasMatch = [];
    this.errorMessages = [];
    if (inputValue.length > 1) {
      this.listEmpresasMatch = this.BuscarMatch(
        this.empresasRazonSocial,
        inputValue
      );
      ...
      if (index == -1)
        this.errorMessages.push(
          "No hay empresas registradas con esa razon social."
        );
    }
  }
