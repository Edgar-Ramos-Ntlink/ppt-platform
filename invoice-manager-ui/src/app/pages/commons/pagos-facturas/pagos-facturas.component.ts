...
export class PagosFacturaComponent implements OnInit {
    ...
    public updateDataTable(currentPage?: number, pageSize?: number) {
        ...
        this.paymentsService
            .getAllPayments(pageValue, sizeValue, this.filterParams)
            .pipe(
                map((page: GenericPage<PagoBase>) => {
                    const newPage: GenericPage<TreeNode<PagoFacturaModel>> =
                        Object.assign({}, page, { content: undefined });
                    newPage.content = page.content.map((pago: PagoBase) => {
                        const children: TreeNode<PagoFacturaModel>[] =
                            pago.facturas.map((fact: PagoFactura) => {
        ...
        this.filesService
            ...
            .subscribe((file) =>
                this.downloadService.downloadFile(
                    file.data,
                    `${folio}-${data.deudor}-${data.acredor}.pdf`,
                    'application/pdf;'
                )
            );
    }
