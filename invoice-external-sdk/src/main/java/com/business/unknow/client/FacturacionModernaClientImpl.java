...
public class FacturacionModernaClientImpl extends AbstractClient
    ...
    if (response.getStatusInfo().getFamily() == Status.Family.SUCCESSFUL) {
      ...
      throw new FacturaModernaClientException(
          new FacturaModernaErrorMessage(
              ...
              "Error al timbrar en factura moderna"),
          response.getStatus());
    }
  }
  ...
  public FacturaModernaCancelResponseModel cancelar(FacturaModernaRequestModel request)
    ...
    Response response =
        post(endpoint, MediaType.TEXT_PLAIN, soapRequest.createCancelRequest(request));
    return parseResponse(response, FacturaModernaCancelResponseModel.class);
  }

  @Override
  public FacturaModernaResponseModel stamp(FacturaModernaRequestModel request)
      throws FacturaModernaClientException {
    FacturaModernaMessageParser soapRequest = new FacturaModernaMessageParser();
    log.info("Stamping the invoice.");
    String endpoint = FacturacionModernaEndpoints.getTimbradoEndpoint();
    Response response =
        post(endpoint, MediaType.TEXT_PLAIN, soapRequest.createStampRequest(request));
