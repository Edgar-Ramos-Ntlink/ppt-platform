...
public class RestSwSapiensClientImpl extends AbstractClient implements RestSwSapiensClient {
  ...
  protected <T> T parseResponse(Response response, TypeReference<T> entityType)
    ...
    try {
        ...
        if (content.contains("message")) {
          try (StringReader reader = new StringReader(content)) {
            SwSapiensErrorMessage error =
                mapper.readValue(reader, new TypeReference<SwSapiensErrorMessage>() {});
            log.info(error.toString());
            throw new SwSapiensClientException(error, status);
          }
        } else {
          SwSapiensErrorMessage error =
              new SwSapiensErrorMessage(
                  "Unexpected services response", String.format("Error response: %s", content));
          throw new SwSapiensClientException(error, status);
        }
      }
    } catch (IOException e) {
      ...
      error.setMessage(
          "Client Error, the deserialization of entity "
              + entityType.getClass().getName()
              + " can't be done.");
      ...
      if (response.getStatusInfo().getFamily() == Status.Family.SUCCESSFUL) {
        if (!content.contains("\"201\"")) {
          try (StringReader reader = new StringReader(content)) {
            SwSapiensErrorMessage error =
                new SwSapiensErrorMessage(
                    "Error canceling the invoice", String.format("Error response: %s", content));
            throw new SwSapiensClientException(error, status);
          }
        }
      } else {
  ...
  public SwSapiensConfig stamp(String xml, String version) throws SwSapiensClientException {
    ...
    String raw =
        SwSapiensConstans.TOKEN_SEPARATOR
            + boundary
            + SwSapiensConstans.TOKEN_SUFIX
            + xml
            + SwSapiensConstans.TOKEN_ENTER
            + boundary
            + SwSapiensConstans.TOKEN_SEPARATOR;
    MultivaluedMap<String, Object> headers = new MultivaluedHashMap<>();
    headers.add(SwSapiensConstans.TOKEN_PARAMETER, token);
    headers.add(
        SwSapiensConstans.CONTENT_TYPE_PARAMETER,
        SwSapiensConstans.CONTENT_TYPE_VALUE.concat(boundary));
    headers.add(
        SwSapiensConstans.CONTENT_DISPOSITION_PARAMETER,
        SwSapiensConstans.CONTENT_DISPOSITION_VALUE);
    String endpoint = SwSapiensEndpoints.getStampByVersionEndpoint(version);
    Response response =
        post(endpoint, SwSapiensConstans.CONTENT_TYPE_VALUE.concat(boundary), raw, headers);
    return parseResponse(response, new TypeReference<SwSapiensConfig>() {});
  }
