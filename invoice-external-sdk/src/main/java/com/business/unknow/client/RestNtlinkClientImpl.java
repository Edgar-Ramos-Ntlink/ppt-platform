...
public class RestNtlinkClientImpl extends AbstractClient implements RestNtlinkClient {
  ...
  protected <T> T parseCancelResponse(Response response, Class<T> clazz)
  ...
  @Override
  public NtlinkCancelResponseModel cancelar(NtlinkCancelRequestModel request)
      throws NtlinkClientException {
    NtlinkMessageParser soapRequest = new NtlinkMessageParser();
    ...
    headers.add(
        "SOAPAction", "https://ntlink.com.mx/IServicioTimbrado/IServicioTimbrado/CancelaCfdi");
    Response response =
        post(endpoint, MediaType.TEXT_XML, soapRequest.createCancelRequest(request), headers);
    NtlinkCancelResponseModel ntlinkResponse =
        parseCancelResponse(response, NtlinkCancelResponseModel.class);
    if (ntlinkResponse.getCancelaCfdiResult() != null
        && ntlinkResponse.getCancelaCfdiResult().contains("Error al cancelar el comprobante")) {
      throw new NtlinkClientException(
          new NtlinkErrorMessage(
              "Error al cancelar en Ntlink", ntlinkResponse.getCancelaCfdiResult()),
          response.getStatus());
    } else {
  ...
  public NtlinkResponseModel stamp(NtlinkRequestModel request) throws NtlinkClientException {
    ...
    Response response =
        post(endpoint, MediaType.TEXT_XML, soapRequest.createStampRequest(request), headers);
    NtlinkResponseModel ntlinkResponse = parseResponse(response, NtlinkResponseModel.class);
    if (ntlinkResponse.getDescripcionError() != null
        && !ntlinkResponse.getDescripcionError().isEmpty()) {
      throw new NtlinkClientException(
          new NtlinkErrorMessage(
              "Error al timbrar en Ntlink", ntlinkResponse.getDescripcionError()),
          response.getStatus());
    } else {
      return ntlinkResponse;
    }
