...
@Rule(
    name = CancelacionSuite.CANCELAR_STATUS_VALIDATION,
    description = CancelacionSuite.CANCELAR_STATUS_VALIDATION_RULE)
public class CancelarStatusValidationRule {
  ...
  public void execute(@Fact("facturaContext") FacturaContext fc) {
    ...
    fc.setSuiteError(
        String.format("Error durante : %s", CancelacionSuite.CANCELAR_STATUS_VALIDATION_RULE_DESC));
    fc.setValid(false);
  }
}
